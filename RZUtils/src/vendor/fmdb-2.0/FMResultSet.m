...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        RZLog(RZLogError,@"Warning: There seem to be no columns in this set.");
    }
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        RZLog(RZLogError,@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        RZLog(RZLogError,@"Database busy");
        if (outErr) {
        ...
        RZLog(RZLogError,@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
        ...
        // wtf?
        RZLog(RZLogError,@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    RZLog(RZLogError,@"Warning: I could not find the column named '%@'.", columnName);
