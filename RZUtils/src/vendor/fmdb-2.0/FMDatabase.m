...
- (BOOL)open {
    ...
    if(err != SQLITE_OK) {
        RZLog(RZLogError,@"error opening!: %d", err);
        return NO;
    }
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) !=0) {
                    RZLog(RZLogError,@"Closing leaked statement");
            ...
            RZLog(RZLogError,@"error closing!: %d", rc);
        }
    }
    while (retry);
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    ...
    if (delta < [self maxBusyRetryTimeInterval]) {
        ...
        if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
            RZLog(RZLogWarning,@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built with HAVE_USLEEP=1?", requestedSleepInMillseconds, actualSleepInMilliseconds);
        }
        return 1;
    }
...
- (int)busyRetryTimeout {
    RZLog(RZLogWarning,@"%s:%d", __FUNCTION__, __LINE__);
    RZLog(RZLogWarning,@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    RZLog(RZLogWarning,@"%s:%d", __FUNCTION__, __LINE__);
    RZLog(RZLogWarning,@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        RZLog(RZLogError,@"error on rekey: %d", rc);
        RZLog(RZLogError,@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    RZLog(RZLogWarning,@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        RZLog(RZLogError,@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        RZLog(RZLogInfo,@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                RZLog(RZLogError,@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                RZLog(RZLogError,@"DB Query: %@", sql);
                RZLog(RZLogError,@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
                RZLog(RZLogInfo,@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }
            ...
            if (namedIdx > 0) {
                ...
                RZLog(RZLogError,@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
        ...
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    RZLog(RZLogInfo,@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
                    RZLog(RZLogInfo,@"obj: %@", obj);
                }
            }
    ...
    if (idx != queryCount) {
        RZLog(RZLogError,@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        RZLog(RZLogInfo,@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        RZLog(RZLogError,@"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
            RZLog(RZLogError,@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            RZLog(RZLogError,@"DB Query: %@", sql);
        }
    }
        ...
        // uh oh.
        if (_logsErrors) {
            RZLog(RZLogError,@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            RZLog(RZLogError,@"DB Query: %@", sql);
        }
    }
    else {
        // wtf?
        if (_logsErrors) {
            RZLog(RZLogError,@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            RZLog(RZLogError,@"DB Query: %@", sql);
        }
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            RZLog(RZLogError,@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            RZLog(RZLogError,@"DB Query: %@", sql);
        }
    }
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    if (errmsg && [self logsErrors]) {
        RZLog(RZLogError,@"Error inserting batch: %s", errmsg);
        sqlite3_free(errmsg);
    }
