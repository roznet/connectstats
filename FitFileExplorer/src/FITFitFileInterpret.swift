...
class FITFitFileInterpret: NSObject {
    ...
    func messageFieldForTimestamp(message:String, timestamp:Date) -> FITFitMessageFields?{
        var rv : FITFitMessageFields?
        if let messages:FITFitMessage = self.fitFile[message] {
            for item in messages  {
                if let mf = item as? FITFitMessageFields,
                    let ts = mf["timestamp"]?.dateValue {
                    if( ts == timestamp ){
                        rv = mf;
                        break
                    }
        ...
        if let messages :FITFitMessage = self.fitFile[message] {
            ...
            for item in messages {
                if let fields = item as? FITFitMessageFields{
                    if( fields.hasDate(forKey: "timestamp", after: interval?.from, andStrictlyBefore: interval?.to)){
